...
h2o.on_event(defines.events.on_chunk_generated, function(event)
    ...
    local permutation = {}
    while #basic ~= 0 do
        permutation[#permutation + 1] = table.remove(basic, math.random(#basic))
    end
	...
	for i = 1, 32 do
		...
		for j = 1, 32 do
			...
			if decorative then
				decoratives[#decoratives + 1] = decorative
			end
			if hidden then
				hidden_tiles[#hidden_tiles + 1] = {position, hidden}
			end
		end
	end
	...
	for _, hidden in pairs(hidden_tiles) do
		surface.set_hidden_tile(hidden[1], hidden[2])
	end
...
Mapgen.calculate_noise = function(planet, chunkpos)
	...
	local noise = {
		...
		set_location = function(self, xx, yy)
			self.i = ((xx - x) * 32 + (yy - y)) + 1
		end,
		get_at = function(self, key, xx, yy)
			local i = ((xx - x) * 32 + (yy - y)) + 1
			return result[key][i]
		end
	}
	setmetatable(noise, {
